...
static void CalculateRegisters(
    uint16 target,
    uint16* pCalAry,
    uint8* pSwReg,
    uint8* pCalReg);
    ...
    int vbpIn)
{
    ...
    data[0] = VP880_ACFS_DIS | VP880_ATFS_DIS | VP880_ZXR_DIS |
              VP880_AUTO_SSC_DIS | VP880_AUTO_BAT_SHUTDOWN_EN;
    ...
    VpDeviceIdType devId)
{
    ...
    for (x = 0; x <= 0xFF; x++) {
        ...
        VpMpiCmd(devId, EC_1, VP880_R_FILTER_WRT,
            VP880_R_FILTER_LEN, writeData);
        VpMpiCmd(devId, EC_1, VP880_R_FILTER_RD,
            VP880_R_FILTER_LEN, readData);
        for (i = 0; i < VP880_R_FILTER_LEN; i++) {
